...
const argv = yargs(hideBin(process.argv))
    .option('cleanup', {
        alias: 'c',
        type: 'boolean',
        description: 'Clean up generated images'
    })
    .option('optimize', {
        alias: 'o',
        type: 'boolean',
...
async function processServiceImage(inputPath) {
    const dir = path.dirname(inputPath);
    const name = path.basename(inputPath, '-original.jpg');
    ...
    for (const [size, dimensions] of Object.entries(SIZES.services)) {
        ...
        await sharp(inputPath)
            ...
            .toFile(path.join(dir, `${outputName}.jpg`));
...
async function optimizeImages() {
    ...
    // Process logo
    const logoPath = 'images/kriplytics-logo-original.png';
